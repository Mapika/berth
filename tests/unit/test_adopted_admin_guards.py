...
from berth.daemon import admin_adapters, admin_workloads
...
def _adopted(conn):
    ...
    return dep_store.upsert_adopted(
        ...
        address="127.0.0.1", port=30011, gpu_ids=[7], vram_reserved_mb=1,
        image_tag="external")
...
def test_stop_managed_still_calls_manager(tmp_path):
    ...
    dep = dep_store.create(
        conn, model_id=m.id, backend="vllm", image_tag="i:1",
